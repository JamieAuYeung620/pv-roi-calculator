...
class HouseholdAccumulator:
    ...
    def update_observation_window(self, observed_half_hours: int, min_timestamp_utc: pd.Timestamp, max_timestamp_utc: pd.Timestamp) -> None:
...
def _build_bootstrap_payload() -> dict:
    # Bundled bootstrap asset so the runtime works before the official smart-meter
    # dataset is supplied locally. Rebuild with --input for dissertation use.
    ...
    for profile_id, base in base_profiles.items():
        ...
        archetypes[profile_id] = {
            ...
            "weights": weights,
            "fallback": {
                day_type: _normalize_vector(base[day_type]).round(6).tolist()
                for day_type in DAY_TYPE_ORDER
            },
        }
...
def _write_summary_markdown(payload: dict, summary_path: Path) -> None:
    ...
    for profile_id in EMPIRICAL_PROFILE_IDS:
        ...
        lines.append(
            f"- `{profile_id}`: `{profile_payload.get('households_retained', 'not recorded')}` households"
        )
...
def _find_first_matching(columns: Iterable[str], candidates: Iterable[str]) -> str | None:
...
def _iter_csv_sources(input_path: Path) -> list[tuple[Path, str | None]]:
    ...
    return [(path, None) for path in sorted(input_path.rglob("*.csv"), key=lambda path: _natural_sort_key(str(path))) if path.is_file()]


def _source_label(source: tuple[Path, str | None]) -> str:
    path, member = source
    if member is None:
        return str(path)
...
def _standardize_chunk_long(chunk: pd.DataFrame) -> pd.DataFrame:
    timestamp_col = _find_first_matching(chunk.columns, TIMESTAMP_COLUMN_CANDIDATES)
    household_id_col = _find_first_matching(chunk.columns, HOUSEHOLD_ID_COLUMN_CANDIDATES)
    consumption_col = _find_first_matching(chunk.columns, CONSUMPTION_COLUMN_CANDIDATES)
...
def _standardize_chunk_wide(chunk: pd.DataFrame) -> pd.DataFrame:
    timestamp_col = _find_first_matching(chunk.columns, TIMESTAMP_COLUMN_CANDIDATES)
...
def _read_csv_header(source: tuple[Path, str | None]) -> pd.DataFrame:
...
def _iter_csv_chunks(source: tuple[Path, str | None], chunksize: int) -> Iterable[pd.DataFrame]:
    csv_path, member = source
    if member is None:
...
def _iter_standardized_chunks(source: tuple[Path, str | None], long_rows_per_chunk: int, wide_rows_per_chunk: int) -> Iterable[pd.DataFrame]:
    header = _read_csv_header(source)
    timestamp_col = _find_first_matching(header.columns, TIMESTAMP_COLUMN_CANDIDATES)
    household_id_col = _find_first_matching(header.columns, HOUSEHOLD_ID_COLUMN_CANDIDATES)
    consumption_col = _find_first_matching(header.columns, CONSUMPTION_COLUMN_CANDIDATES)
    ...
    for chunk in _iter_csv_chunks(source, chunksize=chunksize):
        standardized = _standardize_chunk_long(chunk) if is_long else _standardize_chunk_wide(chunk)
        if not standardized.empty:
            yield standardized


def _clean_standardized_chunk(
    chunk: pd.DataFrame,
...
def _accumulate_household_stats(
    ...
    for source in sources:
        print(f"Processing source: {_source_label(source)}")
        for chunk in _iter_standardized_chunks(source, long_rows_per_chunk, wide_rows_per_chunk):
            cleaned = _clean_standardized_chunk(chunk, max_halfhour_kwh=max_halfhour_kwh)
            if cleaned.empty:
                continue
...
def _compute_global_fallbacks(retained: dict[str, HouseholdAccumulator]) -> dict[str, np.ndarray]:
    ...
    for day_type_idx, day_type in enumerate(DAY_TYPE_ORDER):
        ...
        fallback[day_type] = _normalize_vector(mean_vector)
    return fallback


def _assign_archetypes(retained: dict[str, HouseholdAccumulator]) -> dict[str, str]:
...
def _normalized_household_vectors(
    ...
    for month_idx in range(12):
        for day_type_idx, day_type in enumerate(DAY_TYPE_ORDER):
            ...
            out[month_idx, day_type_idx] = _normalize_vector(filled)
    return out


def _build_empirical_payload(
    retained: dict[str, HouseholdAccumulator],
    assignments: dict[str, str],
    min_completeness: float,
    min_complete_hours: int,
    ...
    global_fallbacks = _compute_global_fallbacks(retained)
    normalized_vectors = {
        household_id: _normalized_household_vectors(acc, global_fallbacks)
        for household_id, acc in retained.items()
    }
    ...
    for profile_id in EMPIRICAL_PROFILE_IDS:
        ...
        if member_ids:
            ...
            centroid = np.stack([global_fallbacks["weekday"], global_fallbacks["weekend"]])
            centroid = np.broadcast_to(centroid, (12, 2, 24)).copy()

        weights = {}
        for month in range(1, 13):
            weights[str(month)] = {}
            for day_type_idx, day_type in enumerate(DAY_TYPE_ORDER):
                weights[str(month)][day_type] = _normalize_vector(
        ...
        archetypes[profile_id] = {
            ...
            "weights": weights,
            "fallback": {
                day_type: _normalize_vector(np.mean(centroid[:, day_type_idx, :], axis=0)).round(6).tolist()
                for day_type_idx, day_type in enumerate(DAY_TYPE_ORDER)
            },
        }
...
def build_archetype_asset(
    ...
    if bootstrap:
        ...
        assignments = _assign_archetypes(retained)
        payload = _build_empirical_payload(
            retained=retained,
    ...
    _write_summary_markdown(payload, summary_path)
